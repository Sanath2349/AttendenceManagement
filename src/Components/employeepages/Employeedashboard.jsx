// src/components/employeepages/Employeedashboard.jsx
...
import { loadPunchStatus, punchIn, resetPunchStatus } from "../../redux/slices/employeeSlice";
...
export default function Employeedashboard() {
  ...
  const { punchStatus, lastPunchInDate } = useSelector((state) => state.employees);

  const [currentDate, setCurrentDate] = useState(new Date());
  const [currentTime, setCurrentTime] = useState(new Date());
  const [error, setError] = useState(null);

  // Load punch status when component mounts
  useEffect(() => {
    if (userId) {
      dispatch(loadPunchStatus({ userId }));
    }
  }, [dispatch, userId]);

  // Reset punch status and redirect if punched in
  useEffect(() => {
    if (userId) {
      dispatch(resetPunchStatus({ userId }));
      if (punchStatus === 'punchedIn') {
        navigate('/employeepunchout');
      }
    }
  }, [dispatch, punchStatus, navigate, userId]);

  // ... existing timer and helper functions ...
  useEffect(() => {
  ...
  const handlePunchIn = () => {
    ...
    axios
      .post(`http://52.7.177.12:8000/PunchIn?userid=${userId}`, {
        date_and_time: currentDateTime,
      })
      .then((response) => {
        if (response.status === 200) {
          dispatch(punchIn({ userId }));
          setError(null);
          navigate('/employeepunchout'); // Navigate to punchout page after successful punch in
        } else {
  ...
  // If already punched in, return null while redirecting
  if (punchStatus === 'punchedIn') {
    return null;
  }
          ...
          <div className={styles.timeTrackingSection}>
            <div className={styles.punchinsec}>
              ...
              <button
                ...
                disabled={punchStatus === "punchedIn"}
              >
                {punchStatus === "punchedIn" ? "Punched In" : "Punch In"}
              </button>
            </div>
